...
for data_file in "${data_files[@]}"; do
    # Compile the C program for each data file
    executable_name="cure_auto_${data_file%.txt}"
    mpicc -g -Wall -fopenmp cure_auto.c -std=c99 -o "$executable_name" -lm -DDATA_FILE=\"$data_file\"

    # Check if compilation was successful
    if [ ! -f "./$executable_name" ]; then
        echo "Compilation failed or $executable_name not found."
        exit 1
    fi
